...
class ProductCategoryRow1 extends React.Component {
    render() {
        return (<tr><th colSpan="2">{this.props.category}</th></tr>);
    }
}

class ProductRow1 extends React.Component {
    render() {
        var name = this.props.product.stocked ?
            this.props.product.name :
            <span style={{color: 'red'}}>
...
class ProductTable1 extends React.Component {
    render() {
        var rows = [];
        var lastCategory = null;
        this.props.products.forEach((product) => {
            ...
            if (product.category !== lastCategory) {
                rows.push(<ProductCategoryRow1 category={product.category} key={product.category} />);
            }
            rows.push(<ProductRow1 product={product} key={product.name} />);
            lastCategory = product.category;
        });
        return (
            <table>
